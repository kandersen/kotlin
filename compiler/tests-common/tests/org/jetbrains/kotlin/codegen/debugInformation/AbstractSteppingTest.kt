...
abstract class AbstractSteppingTest : CodegenTestCase() {
    ...
    private companion object {
        ...
        private const val MAIN_CLASS = "org.jetbrains.kotlin.codegen.debugInformation.TestProcessServer"
        private const val TEST_CLASS = "TestKt"
        private const val BOX_METHOD = "box"
        private const val LINENUMBER_PREFIX = "// LINENUMBER: "
    }
    ...
    override fun doMultiFileTest(wholeFile: File, files: List<TestFile>) {
        ...
        val lineNumbers = wholeFile
            .readLines()
            .filter { it.matches("$LINENUMBER_PREFIX.*".toRegex()) }
            .map { line ->
                line.drop(LINENUMBER_PREFIX.length)
                    .trim()
            }.joinToString(" ")
    ...
    private fun invokeBoxInSeparateProcess(classLoader: URLClassLoader, aClass: Class<*>, port: Int): String {
        ...
        return TestProxy(Integer.valueOf(port), aClass.canonicalName, classPath).runTest()
    }
